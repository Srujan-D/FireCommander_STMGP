...
class EnvUtilities(object):
            ...
            for i in range(new_fire_front.shape[0]):
                ...
                if ((int(new_fire_front[i][0]) <= (world_Size - 1)) and (int(new_fire_front[i][1]) <= (world_Size - 1))
                    ...
                    if ([int(new_fire_front[i][0]), int(new_fire_front[i][1])] not in onFire_List) and \
                        ...
                        onFire_List.append([int(new_fire_front[i][0]), int(new_fire_front[i][1]), new_fire_front[i][2]])
        ...
        for i in range(len(raw_sensed_list)):
            if len(unique_list) == 0:
                ...
                for j in range(len(unique_list)):
                    if raw_sensed_list[i][0] == unique_list[j][0] and raw_sensed_list[i][1] == unique_list[j][1]:
                        if raw_sensed_list[i][2] > unique_list[j][2]:
                            unique_list[j] = raw_sensed_list[i]
                        flag = 1
                        break
                if flag == 0:
                    unique_list.append(raw_sensed_list[i])
    ...
    def fire_Sensing(onFire_List, agent_loci, sensed_List, world_size):
        ...
        if onFire_List.shape[0] > 0:
            ...
            raw_sensed_idx = np.random.choice(raw_sensed_idx, int(round(confidence_level * len(raw_sensed_idx))), replace=True)
            ...
            for i in range(len(raw_sensed_idx)):
                tmp_x, tmp_y, _ = onFire_List[raw_sensed_idx[i]]
                try:
                    FOV[int(tmp_x - upper_x), int(tmp_y - upper_y)] = 1
                except:
                    print(">>> Oops! The FOV is out of range.")
                    print(FOV.shape), print(tmp_x - upper_x), print(tmp_y - upper_y)

            # Remove the duplicate fire fronts
            raw_sensed_list = onFire_List[raw_sensed_idx, :]
            if len(raw_sensed_list[:, 0]) > 0:
                int_sensed_list = np.zeros((len(raw_sensed_list[:,0]), 3), dtype=int)
                int_sensed_list[:, 0] = raw_sensed_list[:, 0].astype(int)
                int_sensed_list[:, 1] = raw_sensed_list[:, 1].astype(int)
                int_sensed_list[:, 2] = raw_sensed_list[:, 2].astype(int)
                # uni_int_sensed_list = np.unique(int_sensed_list, axis=0)
                uni_int_sensed_list = EnvUtilities.return_unique(int_sensed_list)
