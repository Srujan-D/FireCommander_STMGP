...
class FireCommanderExtreme(object):
	...
	def kernel_initial(self, sf_initial=1.0, ell_initial=1.0, sn_initial=0.1):
		return sf_initial**2 * RBF(length_scale=ell_initial) + WhiteKernel(noise_level=sn_initial)
	...
	def visualize_map(self, time_passed):
		...
		for i in range(self.perception_agent_num):
			...
			mean, std = fire_contour.predict(np.c_[X.ravel(), Y.ravel()], return_std=True)
			# print(mean, std)
