...
class FireCommanderExtreme(object):
    def __init__(
        ...
        self.fireAreas_Num = (
            5 if fireAreas_Num is None else fireAreas_Num
        )  # number of fire areas
        self.perception_agent_num = (
            3 if P_agent_num is None else P_agent_num
        ...
        if online_vis:
            ...
            self.screen = pygame.display.set_mode(
                (self.world_size, self.world_size), 0, 32
            )
    ...
    def update_gps(self, agent1_id, agent2_id):
        ...
        mixed_gp = mix_GPs(
            [self.agent_state[agent1_id][4], self.agent_state[agent2_id][4]]
        )
    ...
    def env_step(
        ...
        for i in range(self.perception_agent_num):
            ...
            # Sensing
            self.sensed_List[i], FOV = Agent_Util.fire_Sensing(
                self.onFire_List,
                self.agent_state[i],
                self.sensed_List[i],
                self.world_size,
            ...
            if len(self.sensed_List[i]) > 0:
                ...
                self.agent_state[i][4].fit(
                    np.array([[self.sensed_List[i][0], self.sensed_List[i][1]]]),
                    np.array([[self.sensed_List[i][2]]]),
                )
                # except:
        ...
        self.perception_complete = (
            sum(len(self.sensed_List[i]) for i in range(self.perception_agent_num))
            + len(self.pruned_List)
    ...
    def env_visualize(self):
        ...
        for i in range(len(self.agent_state)):
            ...
            if self.agent_state[i][3] == 0:
                ...
                searching_Scope_Upper_Left_Corner = (
                    self.agent_state[i][0] - searching_Scope_X / 2,
                    self.agent_state[i][1] - searching_Scope_Y / 2,
                )
                ...
                pygame.draw.circle(
                    self.screen,
                    (0, 0, 255),
                    (int(self.agent_state[i][0]), int(self.agent_state[i][1])),
                    2,
                )
                pygame.draw.rect(
                    self.screen,
                    (0, 0, 255),
                    Rect(searching_Scope_Upper_Left_Corner, searching_Scope_Size),
                    2,
                )
                ...
                pygame.draw.polygon(
                    self.screen, (128, 0, 128), firefighter_Agent_Vertex
                )
    ...
    def visualize_map(self, time_passed):
        # visualize the gaussian contour of the fire as captured by the perception agents
        # each agent has its last state element as the gaussian distribution of the fire it captures
        ...
        for i in range(self.perception_agent_num):
            ...
            fire_contour = self.agent_state[i][4]

            # get the x and y coordinates of the fire contour
            x = np.linspace(0, self.world_size, 100)
            y = np.linspace(0, self.world_size, 100)
            X, Y = np.meshgrid(x, y)
            ...
            # get the mean and variance of the Gaussian Process
            mean, std = fire_contour.predict(
                np.c_[X.ravel(), Y.ravel()], return_std=True
            )
            # print(mean, std)
            ...
            ax[i][0].scatter(
                self.agent_state[i][0], self.agent_state[i][1], c=color[i], s=10
            )
...
if __name__ == "__main__":
    # initialize the env
    env = FireCommanderExtreme(online_vis=False)
    env.env_init()
