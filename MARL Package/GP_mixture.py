...
def mix_GPs(GPs_sklearn: List[GaussianProcessRegressor]):
    ...
    # Combine the component GPs using the learned gating function
    mixed_kernel = sum(P_z[:, i] * gp.kernel_ for i, gp in enumerate(GPs))
    
    # Create a mixed GP object
    mixed_gp = GP(kernel_=mixed_kernel)  # Modify this line according to your custom GP implementation

    # Compute the combined noise level based on gating weights and WhiteKernel components
    combined_noise_level = np.sum(P_z * np.array([gp.kernel_.k2.noise_level for gp in GPs_sklearn]), axis=1)

    # Add the training points to the mixed GP with the computed noise levels
    mixed_gp.add_training_points(X_train_combined, y_train_combined, σ_noises=combined_noise_level)

    # print(mixed_gp.kernel_, type(mixed_gp.kernel_[0]))
