...
class FireCommanderExtreme(object):
	def __init__(self, world_size=None, duration=None, fireAreas_Num=None, P_agent_num=None, A_agent_num=None, online_vis=False):
		...
		area_wind_speed = [20] * self.fireAreas_Num
	...
	def dummy_env_fire_state(self, time_passed):
		self.fire_propagation(time_passed=time_passed)
	...
	def env_step(self, action, p_vel=5, a_vel=5, vert_vel=2, min_alt=5, max_alt=15, time_passed=1, a_c_threshold=1.5, r_func=None):
		...
		self.fire_propagation(time_passed)          # propagate the fire
	...
	def fire_propagation(self, time_passed):
		# checking fire model setting mode and initializing the fire model
		if self.fire_info[1][9] == 0:  # when using "uniform" fire setting (all fire areas use the same parameters)
		...
		self.visualize_fire(time_passed)

	def visualize_fire(self, time_passed):
		 # Create a figure and axis for visualization
		plt.figure(figsize=(8, 8))
		ax = plt.gca()

		# Plot the terrain or background (you can customize this part)
		# For example, you can plot a grid representing the terrain
		ax.imshow(np.zeros((self.world_size, self.world_size)), cmap=cm.Greens, origin='lower', extent=(0, self.world_size, 0, self.world_size))

		# Plot the fire points on the terrain
		if self.fire_map.shape[0] > 0:
			x_fire = self.fire_map[:, 0]
			y_fire = self.fire_map[:, 1]
			fire_intensity = self.fire_map[:, 2]

			# # I want a GMM to be plotted as a contour plot
			# fire_gmm = GaussianMixture(n_components=self.fireAreas_Num, covariance_type='full', random_state=0)
			# fire_gmm.fit(self.fire_map)

			# gmm_means = fire_gmm.means_
			# gmm_covariances = fire_gmm.covariances_
			# gmm_weights = fire_gmm.weights_

			# x = np.linspace(0, self.world_size, 500)
			# y = np.linspace(0, self.world_size, 500)
			# X, Y = np.meshgrid(x, y)
			...
			# 	mean = gmm_means[i][:2]  # Extract x and y coordinates
			# 	cov = gmm_covariances[i][:2, :2]  # Extract the 2x2 covariance matrix
			# 	intensity = gmm_weights[i]
			# 	mvn = multivariate_normal(mean=mean, cov=cov)
			# 	Z += intensity * mvn.pdf(XY).reshape(X.shape)

			# # Create a heatmap-like plot with color encoding for intensity
			# # plt.figure(figsize=(8, 6))
			# plt.contourf(X, Y, Z, cmap=cm.Oranges, levels=50)  # Adjust the number of levels for smoother plot

			# # Add a colorbar to indicate intensity
			# cbar = plt.colorbar()
			# cbar.set_label('Fire Intensity')

			# plt.title('Gaussian Mixture Model with Fire Intensity')
			# plt.xlabel('X-axis')
			# plt.ylabel('Y-axis')
			# # plt.show()


			# Create a scatter plot with fire intensity as the color
			sc = ax.scatter(x_fire, y_fire, c=fire_intensity, cmap=cm.Oranges, marker='s', s=20, alpha=0.7, norm=colors.LogNorm())

			# Add a colorbar using the mappable object (sc)
			plt.colorbar(sc, label="Fire Intensity")

		# Customize the plot further if needed (e.g., adding color mapping)
		# You can adjust the colormap, color scaling, labels, and more

		# Display the plot
		plt.title("Fire Propagation")
		plt.xlabel("X-coordinate")
		plt.ylabel("Y-coordinate")

		# Save the figure or display it as needed
		plt.savefig(f'viz_fire/5))world_just_fire_{time_passed}')
	...
	# 	# Plot the terrain or background (you can customize this part)
	# 	# For example, you can plot a grid representing the terrain
	# 	ax.imshow(np.zeros((self.world_size, self.world_size)), cmap='Greens', origin='lower', extent=(0, self.world_size, 0, self.world_size))
	...
	# 		# Define a grid over which to interpolate the fire intensity
	# 		x_grid, y_grid = np.meshgrid(np.linspace(0, self.world_size, 100), np.linspace(0, self.world_size, 100))

	# 		# Interpolate fire intensity onto the grid
	# 		fire_intensity_grid = griddata((x_fire, y_fire), fire_intensity, (x_grid, y_grid), method='cubic', fill_value=0)

	# 		# Create a heatmap of the interpolated fire intensity
	# 		im = ax.imshow(fire_intensity_grid, cmap='Oranges', origin='lower', extent=(0, self.world_size, 0, self.world_size))
	...
	# 		plt.xlabel("X-coordinate")
	# 		plt.ylabel("Y-coordinate")
	# 		plt.colorbar(im, label="Fire Intensity")


	# 	# Save the figure or display it as needed
	# 	plt.savefig(f'viz_fire/fire_{time_passed}')
	# 	plt.close()

	def visualize_fire_animation(self):
		# Create a figure and axis for visualization
		fig, ax = plt.subplots(figsize=(8, 8))
		ax.set_title("Fire Propagation")
		...
		def update(frame):
			ax.clear()
			ax.imshow(np.zeros((self.world_size, self.world_size)), cmap=cm.Greens, origin='lower', extent=(0, self.world_size, 0, self.world_size))
			if frame < len(self.fire_frames):
				current_fire_map = self.fire_frames[frame]
				if current_fire_map.shape[0] > 0:
					x_fire = current_fire_map[:, 0]
					y_fire = current_fire_map[:, 1]
					fire_intensity = current_fire_map[:, 2]
					sc = ax.scatter(x_fire, y_fire, c=fire_intensity, cmap=cm.Oranges, marker='s', s=20, alpha=0.7, norm=colors.LogNorm())
					plt.colorbar(sc, label="Fire Intensity")

		# Create the animation
		anim = FuncAnimation(fig, update, frames=len(self.fire_frames), repeat=False)
...
if __name__ == '__main__':
	# initialize the env
	env = FireCommanderExtreme(world_size=500, fireAreas_Num=3)
	# env.env_init()
	...
	for step in tqdm(range(num_episode)):
		...
		env.dummy_env_fire_state(time_passed=step)
